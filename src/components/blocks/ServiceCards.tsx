...
export function ServiceCards({ block }: ServiceCardsProps) {
  const { title, subtitle, serviceIds, categoryId, columns } = block;
  ...
  return (
    <div>
      ...
      <div className={cn('grid gap-6', COLUMN_CLASSES[columns])}>
        {/* Services would be fetched and rendered here */}
        <div className="text-sm text-muted-foreground">
          {serviceIds ? `Service IDs: ${serviceIds.join(', ')}` : 'All services'}
